...
def extract_text_from_pdf(filename):
    loader = AmazonTextractPDFLoader(filename, client=textract_client)
    documents = loader.load()
    text_splitter = RecursiveCharacterTextSplitter(
        separators=["\n\n", "\n", ".", " "], chunk_size=4000, chunk_overlap=100 
    )
...
def get_summary(filename, return_intermediate_steps=False):
    ...
    llm = get_llm()
    docs = extract_text_from_pdf("s3://document-summarization/uploaded_file.pdf")
    # docs = get_docs("uploaded_file.pdf")
